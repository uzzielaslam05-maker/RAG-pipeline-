...
Read the provider's retry interval when available.

Parse the retry duration when necessary.

Convert that duration into a recovery time.

Calculate the displayed time in the browser so it follows the visitor's local timezone.

Fall back to a generic wait message if the provider does not expose usable retry information.
